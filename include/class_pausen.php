...
class pausen{
	...
	public static function check($vergleichszeit){
		...
		foreach($pausen->_array as $pause){
			...
			//eingefügt von milack: Prüfung, ob gearbeitete Zeit über Gesamtarbeitszeit von 12 Stunden liegt
			if(is_array($tmp_pausen)){
			    if($vergleichszeit > 12.0) {
                    $vergleichszeit = 12.0;
                }
				if($minpause <= $vergleichszeit AND $maxpause > $vergleichszeit){
					if(($vergleichszeit - $minpause) < round($tmp_pausen[2]/60,2)) {
					    $return[0] = $vergleichszeit - $minpause;
                    } else {
